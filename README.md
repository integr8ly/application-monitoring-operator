...
3. Create *Release* through github release page, name it as the version number - `0.0.29` and state what's new in the release
4. Login to [quay.io](https://quay.io) with `docker login quay.io`
5. Build docker image for the new version and push it to the `quay.io`
```sh
$ make image/build
$ make image/push
```
